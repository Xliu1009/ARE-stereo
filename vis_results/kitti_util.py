...
def draw_box3d_label_on_bev(image, boxes3d, thickness=0.6, scores=None):
    ...
    for n in range(num):
        ...
        if (x0 < 30 and x1 < 30 and x2 < 30 and x3 < 30):
            ...
            cv2.line(img, (u3, v3), (u0, v0), color, thickness, cv2.LINE_AA)
        # if scores.any():
        # score = str(np.around(scores[n], 2))
        if scores:
            cv2.putText(img, scores[n], (u0, v0), cv2.FONT_HERSHEY_SIMPLEX,
                        thickness, color, 1, 4)
    return img
