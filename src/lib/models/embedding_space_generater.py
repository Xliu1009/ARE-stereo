...
def featuremap2gridpoint(batch, phase='train', opt = None):
    ...
    for b in range(B):
        ...
        for py in range(4):
            ...
            volume_from_heatmap_l.append(torch.nn.functional.grid_sample(featuremap_l[b:b + 1],
                                         grid_l, align_corners=True))  # 1,64,16K,resl***2
            volume_from_heatmap_r.append(torch.nn.functional.grid_sample(featuremap_r[b:b + 1],
                                         grid_r, align_corners=True))  # 1,64,16K,resl***2
